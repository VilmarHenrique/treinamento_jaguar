...
//
// $Id: htmlarea.js,v 1.1 2018/08/23 16:29:36 matheus Exp $



// Creates a new HTMLArea object.  Tries to replace the textarea with the given
// ID with it.
function HTMLArea(textarea, config, path) {
	if (HTMLArea.checkSupportedBrowser()) {
		if (typeof config == "undefined") {
			this.config = new HTMLArea.Config(path);
		} else {
			this.config = config;
		}
...
HTMLArea.Config = function (path) {
  ...
  // URL-s
  JAGUAR_URL = path;
  this.imgURL        = JAGUAR_URL + "jformeditor/images/";
  this.fullscreenURL = JAGUAR_URL;
  this.popupURL      = JAGUAR_URL + "jformeditor/popups/";
  this.debug         = 0;
...
HTMLArea.prototype._insertImage = function() {
  ...
  var man = typeof document.maintenance;
    
    if (man == "object")
    {
      var tem_ds_url = typeof document.maintenance.f_t_ds_url;

      if (tem_ds_url == "object")
      {
        if (document.maintenance.f_t_ds_url.value != '' && document.maintenance.f_t_ds_caminho.value != '')
          url_insert_image = "insert_image.php?f_t_ds_url="+document.maintenance.f_t_ds_caminho.value+document.maintenance.f_t_ds_url.value;
        else
          url_insert_image = "insert_image.php";
      }
      else
          url_insert_image = "insert_image.php";
    }
    ...
    this._popupDialog(url_insert_image, function(param) {
...
HTMLArea.prototype._buttonClicked = function(txt) {
  var editor = this;	// needed in nested functions
	...
	switch (cmd.toLowerCase()) {
	    ...
	    case "popupeditor":
		if (HTMLArea.is_ie) {
			window.open(this.fullscreenURL("jformeditor/fullscreen.html"), "ha_fullscreen",
...
HTMLArea.prototype.fullscreenURL = function(file) {
	return this.config.editorURL + this.config.fullscreenURL + file;
};
