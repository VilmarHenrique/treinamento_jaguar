...
var Style=[],Count=0,move=0,fl=0,isOK=1,hs,e_d,tb,w=window,PX=(MI_pSub)?"px":"";
...
function stm(t,s,width,height){
if(MI_sNav&&isOK){
if(document.onmouseover!=mig_mo||w.onresize!=mig_re) mig_hand()
...
var title=t[0]||hs==3?"<TABLE STYLE='background-color:#CECEFF;' WIDTH='100%' BORDER='0' CELLPADDING='0' CELLSPACING='0' "+titBgCol+" "+titBgImg+"><TR><TD "+titTxtAli+"><FONT SIZE='"+s[5]+"' FACE='"+s[4]+"' "+titCol+"><B>"+t[0]+"</B></FONT></TD>"+closeLink+"</TR></TABLE>":"";
var txt="<TABLE STYLE='background-color:#F0F0F0;' "+ab+" "+tipWidth+"' BORDER='0' CELLSPACING='0' CELLPADDING='"+s[14]+"' "+brdCol+"><TR><TD>"+title+"<TABLE WIDTH='100%' "+tipHeight+" BORDER='0' CELLPADDING='"+s[16]+"' CELLSPACING='0' "+txtBgCol+" "+txtBgImg+"><TR><TD "+txtTxtAli+" "+ap+" VALIGN='top'><FONT SIZE='"+s[11]+"' FACE='"+s[10]+"' "+txtCol +">"+t[1]+"</FONT></TD></TR></TABLE></TD></TR></TABLE>"
mig_wlay(txt)
...
*/
function cep_cadastre(cepNumber, nmObjeto)
{
  var complUf     = "";
  var complCidade = "";

  if (nmObjeto != "" && $("." + nmObjeto).length)
  {
    var uf         = "";
    var cidade     = "";
    var dadoObjeto = ($("." + nmObjeto + " option:selected").text()).replace(" / ", "/");

    if (dadoObjeto != "")
    {
      var arrayObjeto = dadoObjeto.split("/");

      if (dadoObjeto.indexOf("/") == 2)
      {
        uf     = arrayObjeto[0];
        cidade = arrayObjeto[1];
      }
      else
      {
        cidade = arrayObjeto[0];

        if (arrayObjeto[1] != undefined)
          uf = arrayObjeto[1];
      }

      if (uf != "")
      {
        complUf = "uf=" + uf + "\">" + uf + "</option";
  ...
  if (cepNumber != '')
    s = 'http://www.buscacep.correios.com.br/servicos/dnec/consultaLogradouroAction.do'+
        '?Metodo=listaLogradouro&TipoConsulta=cep&StartRow=1&EndRow=10&CEP=' + cepNumber;
  else
    s = 'http://www.buscacep.correios.com.br/sistemas/buscacep/buscaCep.cfm?' + complUf + complCidade;
...
{
  value       = (field.value ? field.value : '').replace(/_/g, '');
  field.value = '';
  valor       = new Array;
...
function validate_placa(object, error, formName)
{
  ...
  placa = new RegExp("^[a-zA-Z]{3}-[0-9].[0-9]{2}$");
...
function rm_double_spaces(e, obj)
{
  var code = getKeyCode(e);
  ...
  // Se o campo estiver vazio, não permitir
  // a inserção de espaço
  if (obj.value == '')
    return false;

  try {
    // Obter a posição atual do cursor no campo
    var posCursor = obterPosicaoCursor(obj);

    // Por garantia, retornar se a posição do cursor
    // não tiver sido obtida
    if (posCursor == null || posCursor == undefined)
      return true;

    // Se o caracter imediatamente antes ou imediatamente após o cursor for um espaço,
    // não permitir a inserção de outro espaço. Se a posição do cursor for zero
    // (usuário está tentando inserir um espaço no início), também não permitir
    if ((posCursor == 0)
        || (obj.value.slice(posCursor - 1, posCursor) == ' ')
        || (obj.value.slice(posCursor, posCursor + 1) == ' '))
      return false;
  } catch (e) {
    return true;
  }
...
function obterPosicaoCursor(objField) {
  ...
  if (document.selection) {
    ...
    objSel.moveStart('character', -objField.value.length);

    // The caret position is selection length
    caretPos = objSel.text.length;
  }
  else if (objField.selectionStart || objField.selectionStart == '0')
    caretPos = objField.selectionStart;
...
// Retorna o número formatado conforme os parâmetros precisao e to.
// Se a formatação for para sys, a precisão servirá apenas para truncar o número
// caso ele tenha mais casas decimais que a precisão especificada (a função retornará
// um float para formato sys)
function formatNum(valor, precisao, to) {
  to       = to || "sys";
  precisao = (precisao || precisao == 0) ? precisao : 2;

  if (to == "pt_BR") {
    return $.formatNumber(parseFloat(valor), {
      format: '#,##0.' + new Array((precisao) + 1).join('0'),
      decimalChar: ',',
      thousandsChar: '.'
    });
  } else if (to == "sys") {
    ...
    valor = parseFloat(transform_value(valor));
